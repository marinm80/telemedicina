  ...
  return {
    ...
    TRIAGE_DECISION: {
      ...
      next: (_input: string, ctx: AgentContext) => {
        // Triage rules (no LLM)
        if (ctx.patientData.symptomsSeverity === 'severo' && ctx.patientData.symptomsOnset === 'hoy') {
          ctx.triageResult = 'presencial';
          return 'SUGGEST_PRESENCIAL';
        }
        ctx.triageResult = 'teleconsulta';
